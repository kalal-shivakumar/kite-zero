...
# SUBSCRIPTION (trading-decision feed): ALWAYS driven by TradingSymbol in input.json so the
# breakout decisions track that underlying (e.g. Nifty spot). InstrumentToken is ignored here.
$preset = Resolve-KiteSymbol $cfg.TradingSymbol
if(-not $preset){ Write-Host "  ERROR: TradingSymbol '$($cfg.TradingSymbol)' not in preset table. Cannot subscribe." -f Red; exit 1 }
...
# Detect any already-open position up front (retry until /orders responds) so it shows before the first candle forms.
for($i=0;$i -lt 10;$i++){ $init=Open-Orders; if($null -ne $init){ $g.Trend=$init; break }; Start-Sleep -Milliseconds 300 }
Draw $hist $raw $ltp $tb
# Stay alive across websocket drops: if the socket leaves Open, dispose and reconnect instead of exiting (would abandon the live position).
while($true){
    if($ws.State -ne 'Open'){
