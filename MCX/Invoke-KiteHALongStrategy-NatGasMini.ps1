<#
.SYNOPSIS
  Heikin-Ashi Long-only strategy for Natural Gas Mini (MCX).
.DESCRIPTION
  Streams live HA candles via Kite WebSocket for NATGASMINI. When the current
  HA candle's Close crosses above the previous HA candle's High, a Long Entry
  file is created. When it crosses below the previous HA candle's Low, a Long
  Exit file is created. Only one Long position at a time.
  Order files are saved to PlacedOrders/ in the script directory.
.EXAMPLE
  .\Invoke-KiteHALongStrategy-NatGasMini.ps1
  .\Invoke-KiteHALongStrategy-NatGasMini.ps1 -TimeFrame 3minute
  .\Invoke-KiteHALongStrategy-NatGasMini.ps1 -TimeFrame 5minute -FullMode
#>
