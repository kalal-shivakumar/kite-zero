...
# Option tracking & stoploss state
$script:PE_StopLoss      = 0
$script:PE_OptionLTP     = 0
$script:PE_OptionLow     = [double]::MaxValue
$script:PE_OptionHigh    = 0
$script:WS_Client        = $null
$script:WS_CTS           = $null
...
if (Test-Path $PositionFile) {
    ...
    if ($cleanup -eq 'y' -or $cleanup -eq 'Y') {
        ...
        # Recalculate stoploss from option candle data
        if ($script:PE_EntryToken -gt 0) {
            $optCandles = Get-ZerodhaCandleData -instrument_token $script:PE_EntryToken -tradingsymbol $script:PE_EntrySymbol -TimeFrame "1" -LastNCandles 5
            if ($optCandles -and $optCandles.Count -ge 3) {
                $last3 = $optCandles[-3..-1]
                $script:PE_StopLoss = ($last3 | Measure-Object -Property low -Minimum).Minimum
                Write-Host "  Restored StopLoss: $($script:PE_StopLoss) (Low of last 3 option candles)" -ForegroundColor Magenta
            }
        }
    }
}
...
# ================================================================
# REST API: Fetch historical candle data for options
# ================================================================
function script:Get-ZerodhaCandleData {
    param(
        $tradingsymbol,
        $instrument_token,
        [string]$TimeFrame = "1",
    ...
    if ((-not $script:ShortOrderPlaced) -and ($liveHA.Close -lt $previousCandle.Low)) {
        ...
        if ($atmOption) {
            ...
            if ($result) {
                ...
                # ── Calculate StopLoss from last 3 candles low of the option ──
                $optCandles = script:Get-ZerodhaCandleData -instrument_token $atmOption.Token -tradingsymbol $atmOption.Symbol -TimeFrame "1" -LastNCandles 5
                if ($optCandles -and $optCandles.Count -ge 3) {
                    $last3 = $optCandles[-3..-1]
                    $script:PE_StopLoss = ($last3 | Measure-Object -Property low -Minimum).Minimum
                    Write-Host "  [$($now.ToString('HH:mm:ss.fff'))] STOPLOSS SET: $($script:PE_StopLoss) (Low of last 3 option 1min candles)" -ForegroundColor Magenta
                    Write-Host "  [$($now.ToString('HH:mm:ss.fff'))]   Candle lows: $( ($last3 | ForEach-Object { $_.low }) -join ', ' )" -ForegroundColor DarkMagenta
                } else {
                    # Fallback: 20% below entry LTP
                    $script:PE_StopLoss = [Math]::Round($optLTP * 0.80, 2)
                    Write-Host "  [$($now.ToString('HH:mm:ss.fff'))] STOPLOSS FALLBACK: $($script:PE_StopLoss) (20% below entry LTP $optLTP)" -ForegroundColor DarkYellow
                }
                $script:PE_OptionLTP  = $optLTP
                $script:PE_OptionLow  = $optLTP
                $script:PE_OptionHigh = $optLTP

                # ── Subscribe to option token on WebSocket for real-time SL tracking ──
                if ($script:WS_Client -and $script:WS_Client.State -eq [System.Net.WebSockets.WebSocketState]::Open) {
                    try {
                        $subMsg = [System.Text.Encoding]::UTF8.GetBytes('{"a":"subscribe","v":[' + $instToken + ',' + $atmOption.Token + ']}')
                        $script:WS_Client.SendAsync([System.ArraySegment[byte]]::new($subMsg), [System.Net.WebSockets.WebSocketMessageType]::Text, $true, $script:WS_CTS.Token).Wait(5000)
                        $modeMsg = [System.Text.Encoding]::UTF8.GetBytes('{"a":"mode","v":["quote",[' + $instToken + ',' + $atmOption.Token + ']]}')
                        $script:WS_Client.SendAsync([System.ArraySegment[byte]]::new($modeMsg), [System.Net.WebSockets.WebSocketMessageType]::Text, $true, $script:WS_CTS.Token).Wait(5000)
                        Write-Host "  [$($now.ToString('HH:mm:ss.fff'))] Subscribed to option feed: $($atmOption.Symbol) (Token: $($atmOption.Token))" -ForegroundColor Green
                    } catch {
                        Write-Host "  [$($now.ToString('HH:mm:ss.fff'))] Failed to subscribe to option feed: $($_.Exception.Message)" -ForegroundColor DarkYellow
                    }
                }
            } else {
...
# ================================================================
# StopLoss exit — option LTP drops below recorded stoploss
# ================================================================
function script:Execute-StopLossExit([double]$optionLTP) {
    $now = Get-Date
    Write-Host ""
    Write-Host "  [$($now.ToString('HH:mm:ss.fff'))] *** STOPLOSS TRIGGERED *** Option LTP: $optionLTP < StopLoss: $($script:PE_StopLoss)" -ForegroundColor Red

    $exitQty = $script:PE_EntryQty
    Write-Host "  [$($now.ToString('HH:mm:ss.fff'))] PE SELL (STOPLOSS) | Symbol: $($script:PE_EntrySymbol) | Qty: $exitQty" -ForegroundColor Cyan
    $result = Place-ZerodhaOrder -CommonHeader $headers -Type "SELL" -Variety $Variety `
        -Tradingsymbol $script:PE_EntrySymbol -Quantity $exitQty `
        -OrderType $Order_type -Product $Product -Exchange $exchange -Tag "PE-SL" -MarketProtection $MarketProtection

    $tradePnL = ($optionLTP - $script:PE_EntryOptionLTP) * $exitQty
    $script:PE_TotalPnL += $tradePnL
    $pnlColor = if ($tradePnL -ge 0) { 'Green' } else { 'Red' }
    $orderLatency = ((Get-Date) - $now).TotalMilliseconds
    Write-Host "  [$($now.ToString('HH:mm:ss.fff'))] STOPLOSS EXIT in ${orderLatency}ms | Trade P&L: $($tradePnL.ToString('N2')) | Total P&L: $($script:PE_TotalPnL.ToString('N2'))" -ForegroundColor $pnlColor

    $timeStamp = $now.ToString('yyyy-MM-dd_HH-mm-ss')
    $script:StrategySignals.Add("SL-EXIT @ Opt:$optionLTP (SL:$($script:PE_StopLoss)) P&L: $([Math]::Round($tradePnL,2)) ($timeStamp)")

    # Unsubscribe from option token, keep index subscription
    if ($script:WS_Client -and $script:WS_Client.State -eq [System.Net.WebSockets.WebSocketState]::Open) {
        try {
            $unsubMsg = [System.Text.Encoding]::UTF8.GetBytes('{"a":"unsubscribe","v":[' + $script:PE_EntryToken + ']}')
            $script:WS_Client.SendAsync([System.ArraySegment[byte]]::new($unsubMsg), [System.Net.WebSockets.WebSocketMessageType]::Text, $true, $script:WS_CTS.Token).Wait(5000)
        } catch {}
    }

    # Reset ALL position + stoploss state
    $script:ShortOrderPlaced  = $false
    $script:ShortEntryPrice   = 0.0
    $script:ShortEntryTime    = ''
    $script:PE_InPosition     = $false
...
function script:Render-StrategyDisplay([int]$instrumentToken) {
    ...
    if ($script:PE_InPosition) {
        ...
        if ($script:PE_StopLoss -gt 0) {
            $slDist = $script:PE_OptionLTP - $script:PE_StopLoss
            $slPct  = if ($script:PE_OptionLTP -gt 0) { [Math]::Round(($slDist / $script:PE_OptionLTP) * 100, 1) } else { 0 }
            $null = $sb.AppendLine("  OPTION  : LTP: $($script:PE_OptionLTP.ToString('N2'))  |  StopLoss: $($script:PE_StopLoss.ToString('N2'))  |  SL Dist: $($slDist.ToString('N2')) ($slPct%)  |  Low: $($script:PE_OptionLow.ToString('N2'))  High: $($script:PE_OptionHigh.ToString('N2'))")
        }
        if ($null -ne $currentCandle) {
...
function script:Force-ExitAtStopTime {
    if ($script:PE_InPosition -and $script:PE_EntrySymbol) {
        ...
        if ($script:WS_Client -and $script:WS_Client.State -eq [System.Net.WebSockets.WebSocketState]::Open -and $script:PE_EntryToken -gt 0) {
            try {
                $unsubMsg = [System.Text.Encoding]::UTF8.GetBytes('{"a":"unsubscribe","v":[' + $script:PE_EntryToken + ']}')
                $script:WS_Client.SendAsync([System.ArraySegment[byte]]::new($unsubMsg), [System.Net.WebSockets.WebSocketMessageType]::Text, $true, $script:WS_CTS.Token).Wait(5000)
            } catch {}
...
while ($retryCount -le $maxRetries) {
    ...
    try {
        ...
        # If position exists (script restart/reconnect), also subscribe to option token
        if ($script:PE_InPosition -and $script:PE_EntryToken -gt 0) {
            $subOpt = [System.Text.Encoding]::UTF8.GetBytes('{"a":"subscribe","v":[' + $instToken + ',' + $script:PE_EntryToken + ']}')
            $ws.SendAsync([System.ArraySegment[byte]]::new($subOpt), [System.Net.WebSockets.WebSocketMessageType]::Text, $true, $cts.Token).Wait(5000)
            $modeOpt = [System.Text.Encoding]::UTF8.GetBytes('{"a":"mode","v":["quote",[' + $instToken + ',' + $script:PE_EntryToken + ']]}')
            $ws.SendAsync([System.ArraySegment[byte]]::new($modeOpt), [System.Net.WebSockets.WebSocketMessageType]::Text, $true, $cts.Token).Wait(5000)
            Write-Host "  Resumed option feed: $($script:PE_EntrySymbol) (Token: $($script:PE_EntryToken)) | SL: $($script:PE_StopLoss)" -ForegroundColor Green
        }
            ...
            if (($res.MessageType -eq [System.Net.WebSockets.WebSocketMessageType]::Binary) -and ($res.Count -gt 2)) {
                ...
                foreach ($tick in $ticks) {
                    if ($tick.LastPrice -gt 0) {
                        if ($script:PE_InPosition -and $tick.InstrumentToken -eq $script:PE_EntryToken) {
                            # ── Option tick: track LTP and check stoploss ──
                            $script:PE_OptionLTP = $tick.LastPrice
                            if ($tick.LastPrice -lt $script:PE_OptionLow)  { $script:PE_OptionLow  = $tick.LastPrice }
                            if ($tick.LastPrice -gt $script:PE_OptionHigh) { $script:PE_OptionHigh = $tick.LastPrice }
                            if ($script:PE_StopLoss -gt 0 -and $tick.LastPrice -lt $script:PE_StopLoss) {
                                script:Execute-StopLossExit $tick.LastPrice
                            }
                        } else {
                            # ── Index tick: build HA candles + check signals ──
