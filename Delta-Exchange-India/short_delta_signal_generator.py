#!/usr/bin/env python3
"""
Heikin-Ashi Short-only signal generator for Delta Exchange India.
...
Usage:
  python short_signal_generator.py
  python short_signal_generator.py --symbol ETHUSD --timeframe 5m
"""
...
from delta_exchange import load_config, run_ha_short_strategy


def main():
    parser = argparse.ArgumentParser(description="Delta Exchange HA Short Strategy")
    ...
    print()
    print(f"  Starting Delta Exchange HA Short Strategy (Python/WebSocket)...")
    print(f"  Symbol: {symbol} | TimeFrame: {timeframe}")
    print()

    run_ha_short_strategy(
        symbol=symbol,
        timeframe=timeframe,
