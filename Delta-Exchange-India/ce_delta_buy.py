"""
CE (Call) Option Buyer - Delta Exchange India
==============================================
Monitors PlacedOrders/ for Long-Entry-*.txt and Long-Exit-*.txt signal files.
When entry signal appears: gets spot, finds ITM CE, places BUY order.
When exit signal appears: places SELL order to close position.
Works alongside long_delta_signal_generator.py.
"""
...
def get_atm_call(chain, spot, offset=0):
    calls = [o for o in chain if o.get("contract_type") == "call_options"]
    if not calls:
        return None
    strikes = sorted(set(float(c["strike_price"]) for c in calls))
    if not strikes:
        return None
    atm = min(strikes, key=lambda s: abs(s - spot))
    idx = max(0, min(strikes.index(atm) + offset, len(strikes) - 1))
    target = strikes[idx]
    return next((c for c in calls if float(c["strike_price"]) == target), None)
...
def main():
    ...
    atm_offset = int(cfg.get("ATMOffset_CE", -4))
    ...
    pos_file = os.path.join(orders_dir, "CE-Position.json")
    ...
    if os.path.exists(pos_file):
        try:
            ...
            for f_path in glob.glob(os.path.join(orders_dir, "Long-Entry-*.txt")):
        ...
        for pat in ["Long-Entry-*.txt", "Long-Exit-*.txt"]:
            for f_path in glob.glob(os.path.join(orders_dir, pat)):
                try: os.remove(f_path)
                except OSError: pass
        print("  No position. Cleaned stale Long signal files.")

    print()
    print("  " + "=" * 66)
    print(f"  CE-BUY | {underlying} CALL OPTIONS | Expiry: {expiry}")
    ...
    print("  Entry: Long-Entry-*.txt | Exit: Long-Exit-*.txt")
    print("  " + "=" * 66)
    print()
    ...
    while True:
        try:
            ...
            if not in_position:
                entry_files = glob.glob(os.path.join(orders_dir, "Long-Entry-*.txt"))
                for ef in entry_files:
                    if ef in processed_files:
                        continue
                    print(f"\n  [{now_time_str}] \033[33m LONG ENTRY SIGNAL: {os.path.basename(ef)}\033[0m")
                    try:
                        chain = client.option_chain(underlying, expiry)
                    except Exception as e:
                    ...
                    if spot <= 0:
                        ...
                        processed_files.add(ef)
                        continue
                    atm = get_atm_call(chain, spot, atm_offset)
                    if not atm:
                        print("  Could not find ATM CE. Skipping.")
                        processed_files.add(ef)
                        continue
                    opt_ltp = float(atm.get("close", 0) or atm.get("mark_price", 0))
                    print(f"  Spot: {spot} | Strike: {atm['strike_price']} | CE: {atm['symbol']} | LTP: {opt_ltp}")
                    ...
                    try:
                        ...
                        print(f"  \033[32m BUY ORDER PLACED | {atm['symbol']} | Size: {order_size}\033[0m")
                        if result:
            ...
            if in_position:
                exit_files = glob.glob(os.path.join(orders_dir, "Long-Exit-*.txt"))
                for xf in exit_files:
                    if xf in processed_files:
                        continue
                    print(f"\n  [{now_time_str}] \033[33m LONG EXIT SIGNAL: {os.path.basename(xf)}\033[0m")
                    exit_ltp = get_option_ltp(client, entry_symbol)
                    # Delete all exit files BEFORE placing order to prevent duplicates
                    for f_path in glob.glob(os.path.join(orders_dir, "Long-Exit-*.txt")):
                        try: os.remove(f_path)
                        except OSError: pass
                    try:
            ...
            if t - last_api_fetch >= 2.0:
                ...
                if in_position:
                    ...
                    atm = get_atm_call(chain, cached_spot, atm_offset) if cached_spot > 0 else None
            ...
            print(f"  \033[32m|  CE-BUY | {underlying} CALL OPTIONS | Expiry: {expiry:<14}     |\033[0m")
            print(f"  \033[32m+==============================================================+\033[0m")
            ...
            if in_position:
                ...
                print(f"  \033[32m|\033[0m  Status    : \033[33mWAITING FOR EXIT\033[0m                             \033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  Signal    : Long-Exit-*.txt                          \033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  --------------------------------------------------------\033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  Symbol    : \033[36m{entry_symbol:<40}\033[0m      \033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  Strike    : {entry_strike:<42.1f}    \033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  Spot      : {cached_spot:<42,.2f}    \033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  --------------------------------------------------------\033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  Entry LTP : {entry_ltp:<42.2f}    \033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  Curr  LTP : \033[36m{cached_curr_ltp:<41.2f}\033[0m    \033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  Entry Time: {entry_time:<42}    \033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  --------------------------------------------------------\033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  Unrealized: \033[{u_color}m{unrealized:<41.2f}\033[0m    \033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  Total PnL : \033[{p_color}m{pnl_display:<41.2f}\033[0m    \033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  Trades    : {trade_count:<42}    \033[32m|\033[0m")
            else:
                print(f"  \033[32m|\033[0m  Status    : \033[33mWAITING FOR ENTRY\033[0m                            \033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  Signal    : Long-Entry-*.txt                         \033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  --------------------------------------------------------\033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  ATM CE    : \033[36m{cached_atm_symbol:<40}\033[0m      \033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  Strike    : {cached_atm_strike:<42.1f}    \033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  Spot      : {cached_spot:<42,.2f}    \033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  CE Price  : \033[36m{cached_atm_ltp:<41.2f}\033[0m    \033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  --------------------------------------------------------\033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  Total PnL : {total_pnl:<42.2f}    \033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  Trades    : {trade_count:<42}    \033[32m|\033[0m")
                print(f"  \033[32m|\033[0m  Size      : {order_size:<42}    \033[32m|\033[0m")

            print(f"  \033[32m+==============================================================+\033[0m")
            print(f"  \033[32m|\033[0m  [{now_str}] Polling every 100ms                        \033[32m|\033[0m")
            print(f"  \033[32m+==============================================================+\033[0m")
