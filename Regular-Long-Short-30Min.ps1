<#
.SYNOPSIS
  Combined Regular-candle Long+Short Signal with CE+PE Option Auto-Trade (zero-latency).
.DESCRIPTION
  Streams live regular OHLC candles via Kite WebSocket. Trades both directions:
  - Long entry (Close > prev High) -> BUY CE, exit when Close < prev Low -> SELL CE
  - Short entry (Close < prev Low) -> BUY PE, exit when Close > prev High -> SELL PE
  Only one direction is active at a time. Identical logic to Heikin-Long-Short-Combined.ps1;
  the ONLY difference is signals use regular candles instead of Heikin-Ashi candles.
.EXAMPLE
  .\Regular-Long-Short-Combined.ps1
  .\Regular-Long-Short-Combined.ps1 -TradingSymbol BANKNIFTY -TimeFrame 5minute
#>
...
if (-not $PSBoundParameters.ContainsKey('OffsetPoints'))            { $OffsetPoints  = if ($null -ne $cfg.OffsetPoints) { [double]$cfg.OffsetPoints } else { 5 } }
if (-not $PSBoundParameters.ContainsKey('MaxRetryCountOnBar'))       { $MaxRetryCountOnBar = if ($cfg.MaxRetryCountOnBar) { [int]$cfg.MaxRetryCountOnBar } else { 10 } }
...
$State = @{
    ...
    UseOffsetCandleLogic    = $true
    MaxRetryCountOnBar      = $MaxRetryCountOnBar
    StartTime               = $StartTime
    StopTime                = $StopTime
...
#   Update-RegularStrategyFromTick, Show-RegularStrategyDisplay,
...
Write-Host '  COMBINED: Regular Long+Short | CE+PE Auto-Trade (Zero Latency)' -ForegroundColor Cyan
Write-Host '  ============================================================' -ForegroundColor Cyan
            ...
            if (($res.MessageType -eq [System.Net.WebSockets.WebSocketMessageType]::Binary) -and ($res.Count -gt 2)) {
                ...
                foreach ($tick in $ticks) {
                    if ($tick.LastPrice -gt 0) {
                        Update-RegularStrategyFromTick $State $tick.InstrumentToken $tick.LastPrice $tick.Volume $tick.DayOpen $tick.DayHigh $tick.DayLow $tick.DayClose $tick.OpenInterest
                    }
                }
                try { Show-RegularStrategyDisplay $State $instToken } catch {}
            }
        }
