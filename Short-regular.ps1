<#
.SYNOPSIS
  Combined Regular Short Signal + PE Option Auto-Trade (zero-latency).
.DESCRIPTION
  Streams live Regular candles via Kite WebSocket. When the Regular short entry
  condition fires, immediately places a PE BUY order in the same tick
  — no file I/O, no polling delay. Exit is equally instant.

  Same logic as PUT-Hedge.ps1 but uses regular candles instead of Heikin-Ashi.
  Entry: current Close < previous Low
  Exit:  current Close > previous High
.EXAMPLE
  .\Short-regular.ps1
  .\Short-regular.ps1 -TradingSymbol BANKNIFTY -TimeFrame 5minute
  .\Short-regular.ps1 -TradingSymbol SENSEX -IndexChoosen SENSEX -TimeFrame 15second
#>
...
function script:Check-ShortAndTrade([int]$instrumentToken, [double]$lastPrice) {
    ...
    if (($downTrendStatus.Running -ne $true) -and ($liveClose -lt $previousCandle.Low)) {

        $script:ShortEntryPrice  = $lastPrice

        Write-Host ""
        Write-Host "  [$($now.ToString('HH:mm:ss.fff'))] *** SHORT ENTRY SIGNAL *** LTP: $lastPrice | Close: $([Math]::Round($liveClose,2)) < Prev Low: $($previousCandle.Low)" -ForegroundColor Yellow
    ...
    if (($downTrendStatus.Running -eq $true) -and ($liveClose -gt $previousCandle.High)) {
        $pnl = $script:ShortEntryPrice - $lastPrice

        Write-Host ""
        Write-Host "  [$($now.ToString('HH:mm:ss.fff'))] *** SHORT EXIT SIGNAL *** LTP: $lastPrice | Close: $([Math]::Round($liveClose,2)) > Prev High: $($previousCandle.High)" -ForegroundColor Yellow
...
function script:Render-StrategyDisplay([int]$instrumentToken) {
    ...
    $null = $sb.AppendLine('')
    $null = $sb.AppendLine("  ================================================")
    $null = $sb.AppendLine("  $($config.SymbolLabel) - Regular Short + PE Auto-Trade (COMBINED)")
    $null = $sb.AppendLine("  ================================================")
...
Write-Host '  COMBINED: Regular Short Signal + PE Auto-Trade (Zero File Latency)' -ForegroundColor Cyan
Write-Host '  ============================================================' -ForegroundColor Cyan
